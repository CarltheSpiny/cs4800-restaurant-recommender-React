...
const RestaurantSearch = (props) => {
  ...
  const [renderCard, setShouldRenderCard] = useState(false)
  ...
  useEffect(() => {
    ...
    const fetchReccomendation = async () => {
      try {
        // If the query is empty, dont fetch
        ...
        // If we have fetched before, don't do it again until changed
        if (hasFetched) {
          console.log("Has fetched a response and query has not changed; Not fetching")
          return;
        }

        // If the search button has been clicked, thats when we actually start
        if (!hasClickedSearch) {
          console.log("The query was not submitted (Query may exist); We shouldn't fetch")
          return;
        }

        console.log("Fetching reccomnedation from search")
        const response = await fetch(apiUrl, requestOptions)
        const data = await response.json()

        console.log("Restaurant Search: Error: ", data)
        ...
        console.error("Restuarnt Search: Error: ", error)
        setError(true)
      } finally {
        setLoading(false)
      }
  ...
  if (isError) {
    ...
    console.log("Card should render; Rendering ")
    return (
      <div className="restaurant-search-container">
        <Helmet>
